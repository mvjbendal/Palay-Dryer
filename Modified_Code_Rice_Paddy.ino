...
int countdown_time = 900.0; // 15 minutes in seconds
int convcountdow_time = 600.0;
...
void loop() {
  ...
  if (OPER_FLAG) {
    ...
    if (!countdownStarted) {
      countdown = countdown_time; // Start countdown from 15 minutes
    ...
    if (!convStarted) {
      ...
      // 🔁 Replaced humidity check with countdown check
      if (countdown == 0) {
        HEATING_OFF();
        RELEASE();
        VERT_UP();
...
void OPER_START() {
  ...
  if (millis() - lastCountdownTime >= countdownInterval) {
    lastCountdownTime = millis();
    if (countdown > 0) {
      countdown--; // ⬅ Countdown now decreases
    }
  }
}
